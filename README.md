# :service_dog:  프리미엄 반려동물 호텔링 서비스 `Moongtel` 
<br>
...
2023.03 ~ 04 : [2.0v 으로 리팩토링](https://github.com/penrose15/MOONGTEL-dog-hotel-reservation-v2.0)
...
- [x] Backend
  - ERD 설계 참여
  - 호텔 홍보글 업로드/수정/조회/삭제
    - S3를 활용하여 이미지 업로드 기능 구현
  - 태그기능 구현
    - 태그 추가/수정/삭제 구현
  - 검색 기능(제목/본문 검색, 태그 검색)
  - 메인 페이지 정렬
  - 예약 기능 구현
    - 예약 전 예약 가능 여부 확인(남은 방 개수 카운트) 로직 구현
    - 예약과 함께 강아지 정보 업체로 전송
  - AWS(EC2, RDS, S3)를 활용하여 서버 배포
  - CI/CD 구축
    - Git push시 Jenkins를 통하여 CI/CD 일어남

## 개발에 어려웠던 점 / 아쉬웠던 점
  - [x] ERD 설계 오류로 구현에 살짝 어려움이 있었다.
  - [x] 짧은 시간동안 구현을 하다보니 클린 코드에는 신경쓰지 못한 점이 아쉬웠다.

## 느낀점
  - [x] 팀프로젝트를 진행하며, 협업도구의 중요성을 깨달았다.
  - [x] 가독성 좋은 코드는 유지보수 및 협업에 중요하다는 것을 깨닫게 되었다.
  - [x] 초반의 제대로 된 ERD 설계가 중요하다는 사실을 알게 되었다.
